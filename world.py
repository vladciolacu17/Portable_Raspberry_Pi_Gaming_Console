...
class World:
	...
	# Update `_handle_traps` in `world.py`
	def _handle_traps(self):
		player = self.player.sprite

		if player.trap_cooldown > 0:
			player.trap_cooldown -= 1  # Decrease cooldown timer

		for sprite in self.traps.sprites():
			if sprite.rect.colliderect(player.rect) and player.trap_cooldown == 0:
				if player.direction.x < 0 or player.direction.y > 0:
					player.rect.x += tile_size
				elif player.direction.x > 0 or player.direction.y > 0:
					player.rect.x -= tile_size
				player.life -= 1
				player.trap_cooldown = 30  # Set cooldown to prevent repeated hits
